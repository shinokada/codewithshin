...
test('svelte-svg-icon-sets page has expected h1, meta title', async ({ page }) => {
	await expect(
		page.getByRole('heading', { name: 'Svelte SVG Icon & Illustration Sets' })
	).toBeVisible();
});
...
test('svelte-svg-icon-sets page has expected meta description', async ({ page }) => {
	...
	await expect(metaDescription).toHaveAttribute(
		'content',
		'Svelte SVG Icon and Illustration Sets for for Svelte 4/5/Runes projects'
	);
});
...
test('svelte-svg-icon-sets page has expected meta og tags', async ({ page }) => {
	...
	await expect(metaOgDescription).toHaveAttribute(
		'content',
		'29 Svelte SVG Icon Sets for Svelte 4/5/Runes projects'
	);
...
test('svelte-svg-icon-sets page has expected meta twitter tags', async ({ page }) => {
	...
	await expect(metaTwitterTitle).toHaveAttribute(
		'content',
		'Svelte SVG Icon Sets - codewithshin.com'
	);
	const metaTwitterDescription = page.locator('meta[name="twitter:description"]');
	await expect(metaTwitterDescription).toHaveAttribute(
		'content',
		'Svelte SVG Icon & Illustration Sets for for Svelte 4/5/Runes projects'
	);
